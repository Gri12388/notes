...
export const setSession = async (user, expire) => {
    let result = NOTHING;
    try {
        const db = Pg.getInstance().getPg();
        ...
        if (rows.length > 0)
            result = checkNumber(rows[0].id)?.toString() ?? NOTHING;
        else
            console.warn("[warn", locale.emptyArray);
    }
    catch (error) {
...
export const delSessionByUser = async (user) => {
    let result = false;
    try {
        const db = Pg.getInstance().getPg();
